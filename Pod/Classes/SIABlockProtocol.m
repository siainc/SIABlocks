...
- (SIABlockProtocol *)sia_blockProtocolForProtocol:(Protocol *)protocol
{
    ...
    @synchronized(self) {
        blockObject = objc_getAssociatedObject(self, (__bridge const void *)protocol);
        if (blockObject == nil) {
            blockObject = [SIABlockProtocol createSubclassInstanceForProtocol:protocol];
            objc_setAssociatedObject(self, (__bridge const void *)protocol, blockObject, OBJC_ASSOCIATION_RETAIN);
        }
    }
    return blockObject;
}
...
- (void)sia_implementDelegateUsingBlock:(void (^)(SIABlockProtocol *protocol))block
{
    [self sia_implementProtocol:[self sia_searchProtocolWithSuffix:@"Delegate"]
                 setterSelector:@selector(setDelegate:)
                     usingBlock:block];
}

- (void)sia_implementDataSourceUsingBlock:(void (^)(SIABlockProtocol *protocol))block
{
    [self sia_implementProtocol:[self sia_searchProtocolWithSuffix:@"DataSource"]
